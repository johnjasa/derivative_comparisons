...
class MatrixComp(om.ExplicitComponent):
    ...
    def compute_jacvec_product(self, inputs, d_inputs, d_outputs, mode='fwd'):
        num_inputs = self.options['num_inputs']
        num_outputs = self.options['num_outputs']
        bandwidth = self.options['bandwidth']
        x = inputs['x']
        dx = onp.zeros(x.shape)
        dx[:] = d_inputs['x']
        
        primals = (x, self.random_array, num_inputs, num_outputs, bandwidth)
        tangents = (dx, self.random_array, num_inputs, num_outputs, bandwidth)
        
        print(primals)
        print(tangents)
        
        d_outputs['y', 'x'] = jax.jvp(compute_func, primals, tangents)